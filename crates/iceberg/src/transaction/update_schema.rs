...
use crate::spec::{
    is_promotion_allowed, ListType, MapType, NestedField, NestedFieldRef, PrimitiveType, Schema,
    StructType, Type,
};
...
impl UpdateSchemaAction {
    ...
    fn internal_add_column(
        ...
        let (parent_id, full_name) = if let Some(parent_name) = parent {
            ...
            if self.deletes.contains(&actual_parent.id) {
                return Err(Error::new(
                    ErrorKind::DataInvalid,
                    format!("Cannot add to a column that will be deleted: {}", parent_name),
                ));
            }
            ...
            let parent_name_str = self
                ...
                .ok_or_else(|| {
                    Error::new(
                        ErrorKind::DataInvalid,
                        format!("Cannot find parent column name for id: {}", actual_parent.id),
                    )
                })?;
            ...
            if let Ok(existing) = self.find_field(&existing_field_name) {
                if !self.deletes.contains(&existing.id) {
                    return Err(Error::new(
                        ErrorKind::DataInvalid,
                        format!("Cannot add column, name already exists: {}.{}", parent_name, name),
                    ));
                }
            }
    ...
    pub fn delete_column(mut self, name: impl AsRef<str>) -> Result<Self> {
        ...
        if self.parent_to_added_ids.contains_key(&field.id) {
            return Err(Error::new(
                ErrorKind::DataInvalid,
                format!("Cannot delete a column that has additions: {}", name.as_ref()),
            ));
        }
    ...
    pub fn rename_column(
        ...
        if self.deletes.contains(&field.id) {
            return Err(Error::new(
                ErrorKind::DataInvalid,
                format!("Cannot rename a column that will be deleted: {}", old_name.as_ref()),
            ));
        }
    ...
    pub fn update_column_type(
        ...
        if self.deletes.contains(&field.id) {
            return Err(Error::new(
                ErrorKind::DataInvalid,
                format!("Cannot update a column that will be deleted: {}", name.as_ref()),
            ));
        }
    ...
    fn internal_update_column_requirement(
        &mut self,
        name: &str,
        is_optional: bool,
    ) -> Result<()> {
        ...
        if !is_optional && !self.allow_incompatible_changes {
            return Err(Error::new(
                ErrorKind::DataInvalid,
                format!("Cannot change column nullability: {}: optional -> required", name),
            ));
        }
...
mod tests {
    ...
    fn test_update_column_type_not_allowed() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().message().contains("Cannot change column type"));
    }
